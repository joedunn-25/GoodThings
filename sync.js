...
window.AppSync = {
  ...
  async syncAdd(entry) {
    ...
    const { error } = await this._db.from('entries').upsert(
      { id: entry.id, user_id: this._userId, text: entry.text, timestamp: entry.timestamp },
  ...
  async flushPending() {
    ...
    if (pendingAdds.length) {
      const rows = pendingAdds.map(e => ({
        id: e.id, user_id: this._userId, text: e.text, timestamp: e.timestamp
      }));
  ...
  async pullRemote() {
    ...
    const toUpload = remapped
      ...
      .map(e => ({ id: e.id, user_id: this._userId, text: e.text, timestamp: e.timestamp }));
